...
c.preds <- read_csv("data-processed/c/c.arctic.predictions.fullposts.csv")
c.preds <- as.matrix(c.preds)

lf.preds <- lf.arctic.mod$BUGSoutput$sims.list$z.trait.mu.pred.pop ## Only get the population-level fit
PDR.preds <- PDR.arctic.mod$BUGSoutput$sims.list$z.trait.mu.pred
EFGC.preds <- read_csv("data-processed/EFGC/EFGC.arctic.predictions.fullposts.csv")
...
c.params.fullposts <- read.csv("data-processed/c/c.arctic.params.fullposts.csv")
...
EFGC.params.fullposts <- read.csv("data-processed/EFGC/EFGC.arctic.params.fullposts.csv")
...
# Save output
write.csv(S.viz.out, "data-processed/S.offset.vizout.csv")
...
ggsave("figures/S.offset.viz.png", plot.S.viz, width = 10.3, height = 5.6)
...
ggsave("figures/S.offset.all.png", plot.S.all, width = 10.3, height = 5.6)
