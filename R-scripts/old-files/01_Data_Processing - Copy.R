...
EV.Shocket2020 <- read_csv("data-raw/TraitData_EV_Shocket2020.csv") %>% 
  clean_names()

unique(EV.Shocket2020$host_code)

## Ae. vexans is found in subarctic regions, while Ae. dorsalis and Ae. 
...
EFGC.Mordecai2019 <- read_csv("data-raw/Fecundity_Data_Mordecai2019.csv") %>% 
  clean_names() %>% 
  filter(trait_name == "TFD") # EFGC is named as TFD in this dataset
...
plot.data.EFGC <- TraitData_EFGC %>% 
  ...
  scale_colour_discrete(name = "Citation", labels = c("Delatte 2009",
                                                      "Ezeakacha 2015",
                                                      "Yee 2016"
  )) +
