...
plot.a <- a.alldata.predictions.summary %>% 
  ...
  labs(title = expression(paste("Biting Rate (a)")),
...
plot.bc <- bc.arctic.predictions.summary %>% 
  ...
  # Customize the axes and labels
  scale_x_continuous(limits = c(0, 46)) + 
  labs(title = expression(paste("Vector Competence (bc)")),
       x = expression(paste("Temperature (", degree, "C)")), 
       y = "Proportion") +
  theme_bw() +
  theme(title = element_text(size = 12),
...
plot.lf <- lf.alldata.predictions.summary %>% 
  ...
  # Customize the axes and labels
  scale_x_continuous(limits = c(0, 46)) + 
  labs(title = expression(paste("Adult Lifespan (lf)")),
       x = expression(paste("Temperature (", degree, "C)")), 
       y = "Time (days)") +
  scale_colour_manual(values = c("Arctic" = "black", "non-Arctic" = "azure4")) +
  theme_bw() +
...
plot.PDR <- PDR.arctic.predictions.summary %>% 
  ...
  # Customize the axes and labels
  scale_x_continuous(limits = c(0, 46)) + 
  labs(title = expression(paste("Pathogen Development\nRate (PDR)")),
       x = expression(paste("Temperature (", degree, "C)")), 
...
plot.EFGC <- EFGC.alldata.predictions.summary %>% 
  ...
  theme(title = element_text(size = 12),
        legend.position="none",
        plot.margin = margin(35,5.5,5.5,5.5),
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 14)
        )
...
plot.EV <- EV.arctic.predictions.summary %>% 
  ...
  # Customize the axes and labels
  scale_x_continuous(limits = c(0, 46)) + 
  labs(title = expression(paste("Egg Viability (EV)")),
       x = expression(paste("Temperature (", degree, "C)")), 
...
plot.pLA <- pLA.arctic.predictions.summary %>% 
  ...
  # Customize the axes and labels
  scale_x_continuous(limits = c(0, 46)) + 
  labs(title = expression(paste("Larval-to-Adult Survival\n(pLA)")),
       x = expression(paste("Temperature (", degree, "C)")), 
...
plot.MDR <- MDR.arctic.predictions.summary %>% 
  ...
  # Customize the axes and labels
  scale_x_continuous(limits = c(0, 46)) + 
  labs(title = expression(paste("Mosquito Development\nRate (MDR)")),
       x = expression(paste("Temperature (", degree, "C)")), 
       y = parse(text = "Development~rate~(day^-1)")) +
  theme_bw() +
  theme(title = element_text(size = 12),
...
##### Plot all traits #####
plot.traits <- plot_grid(plot.pLA, plot.MDR, plot.PDR, plot.EV,
                         plot.a + theme(legend.position="none"), plot.lf, 
                         plot.EFGC, plot.bc,
                         ncol = 4,
                         align = "hv",
...
ggsave("figures/CSEE.trait.TPCs.png", plot.traits, width = 12, height = 6)
...
ggsave("figures/CSEE.trait.TPCs.legend.png", legend_panel, width = 5, height = 2)
...
plot.traits.scaled <- prediction.summary %>% 
  ...
  scale_colour_manual(values = c("a" = "#E69F00", 
                      ...
                      breaks = c("a", "bc", "lf", "PDR", "EFGC", "EV", "pLA", "MDR"),
                      labels = c("Biting rate (a)", 
                                 "Vector competence (bc)", 
                                 "Adult lifespan (lf)", 
...
ggsave("figures/CSEE.trait.TPCs.summary.png", plot.traits.scaled, width = 10, height = 4)
