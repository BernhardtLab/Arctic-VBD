...
priors
ggsave("figures/MDR.bri.priors.png", priors, width = 10.3, height = 5.6)
...
priors <- plot_grid(q, T0, Tm, ncol = 3)
priors
ggsave("figures/MDR.quad.priors.png", priors, width = 10.3, height = 5.6)
...
## DIC
MDR.arctic.bri.inf$BUGSoutput$DIC  
MDR.arctic.quad.inf$BUGSoutput$DIC # This is the best-fitting TPC
...
df.arctic.nonarctic <- bind_rows(df.MDR.nonarctic.quad.uni.pop, df.MDR.arctic.quad.inf)
...
MDR.arctic.mod <- MDR.arctic.quad.inf
...
##### Arctic #####
Temp.xs <- seq(0, 45, 0.1)
MDR.TPC.analysis <- extractTPC(MDR.arctic.quad.inf, "MDR", Temp.xs)
